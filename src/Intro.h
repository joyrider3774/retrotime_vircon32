...
void Intro()
{
	...
	if (scene == 0)
	{
		...
		h = tsize*2;
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s), (ScreenWidth - w) / 2, (ScreenHeight - h) /2, tsize, color_white);
		
		Ps = s; *Ps = 0; Ps = faststrcat(Ps,"presents");
		w = CFont_TextWidth("Roboto-Regular", tsize, s, strlen(s));
		h = tsize;
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s), (ScreenWidth - w) / 2, (ScreenHeight) /2, tsize, color_white);
	}
	...
	if (scene == 1)
	{
		...
		w = CFont_TextWidth("Roboto-Regular", tsize, s, strlen(s));
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s), (ScreenWidth - w) / 2, (int)(40.0*yscale), tsize*2, color_white);
		
		Ps = s; *Ps = 0; Ps = faststrcat(Ps,"A game containing");
		w = CFont_TextWidth("Roboto-Regular", tsize, s, strlen(s));
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s),(ScreenWidth - w) / 2, (int)((ScreenHeight - 3.0 * (tsize + 10.0*yscale)) /2.0), tsize, color_white);
		
		itoa(Games, Nr, 10);
		Ps = s; *Ps = 0; Ps = faststrcat(Ps, Nr);
		Ps = faststrcat(Ps," retro based games");
		w = CFont_TextWidth("Roboto-Regular", tsize, s, strlen(s));
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s), (ScreenWidth - w) / 2,(int) ((ScreenHeight - 1 * (tsize + 10*yscale)) / 2.0), tsize, color_white);
	...
	if (scene >= 2)
	{
		...
		w = CFont_TextWidth("Roboto-Regular", tsize, s, strlen(s));
		CFont_WriteText("Roboto-Regular", tsize, s, strlen(s), (ScreenWidth - w) / 2, (int)(40.0*yscale) + tsize*2 + 10 + TmpTz->y/3 + 30, tsize, color_white);
